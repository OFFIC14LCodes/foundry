...
const MENU_ITEMS: Array<{ label: string; sub: string; icon: IconComponent }> = [
  { label: "Navi Academy", sub: "Deep founder learning hub", icon: Icons.sidebar.academy },
  ...
  { label: "Monday Briefings", sub: "Weekly Navi updates", icon: Icons.sidebar.briefings },
...
const TOUR_STEPS: GuideStep[] = [
  ...
  },
  {
    ...
    body: "Talk with Navi stage by stage, work through blockers, complete goals, and get pushed toward the next concrete move.",
    location: "Open it from the Hub by entering your current stage, or tap the Navi bubble when it makes sense.",
  ...
  },
  {
    ...
    kind: "menu",
    eyebrow: "Menu Tool",
    title: "Navi Academy gives you the deeper why.",
    body: "Learn the fundamentals behind what Navi is telling you so you can make sharper decisions on your own.",
    location: "From the Hub, open the top-left menu and choose Navi Academy.",
    useFor: "Use it when you want to study a concept, not just solve the next immediate problem.",
    accent: "var(--tekori-gold)",
    targetLabel: "Navi Academy",
  ...
  },
  {
    ...
    useFor: "Use it when you're changing how Tekori works for you, not what you're building inside it.",
    accent: "#B8B2AA",
        ...
        {TOUR_STEPS.map((_, i) => (
          <div
            ...
            style={{
              ...
              height: 6,
              borderRadius: 3,
              background: i === step ? "var(--tekori-gold)" : "rgba(7,26,47,0.10)",
      ...
      >
        ...
        <button
          ...
          style={{
            ...
            background: isLast ? "linear-gradient(135deg, var(--tekori-gold) 0%, var(--color-accent-hover) 100%)" : "rgba(216,155,43,0.15)",
            border: `1px solid ${isLast ? "transparent" : "rgba(216,155,43,0.35)"}`,
            color: isLast ? "var(--color-primary)" : "var(--tekori-gold)",
          }}
        >
          {isLast ? (
            <>
...
function WelcomeSlide({ name }: { name?: string }) {
  return (
    <div style={{ textAlign: "center", width: "100%", maxWidth: 560 }}>
      ...
      <div style={eyebrowStyle("var(--tekori-gold)")}>Welcome to Tekori</div>
...
function HubGuideSlide({ step }: { step: GuideStep }) {
  ...
  const budgetItems = [
    { label: "Budget", value: "$5k", color: "var(--color-text)" },
    { label: "Income", value: "+$600", color: "var(--color-success)" },
    { label: "Spent", value: "$1.2k", color: "var(--tekori-gold)" },
    { label: "Left", value: "$4.4k", color: "var(--color-success)" },
  ];
          ...
          <div style={{ padding: "14px 16px", display: "grid", gap: 10 }}>
            ...
            <div style={mockCardStyle}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
                ...
                <div style={{ fontSize: 10, color: "var(--tekori-gold)" }}>Stage 2 of 6</div>
              </div>
              <div style={{ display: "flex", gap: 4 }}>
                {stages.map((s, i) => (
                  <div key={i} style={{ flex: 1, textAlign: "center" }}>
                    ...
                    <div style={{ fontSize: 8, color: s.done ? "var(--color-success)" : s.current ? "var(--tekori-gold)" : "var(--color-text-soft)", letterSpacing: "0.02em" }}>{s.label}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div style={mockCardStyle}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
                ...
                <div style={{ fontSize: 10, color: "var(--color-success)" }}>Solid Foundation</div>
              </div>
              <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                <svg width="62" height="62" viewBox="0 0 60 60" style={{ flexShrink: 0 }}>
                  <circle cx="30" cy="30" r={r} fill="none" stroke="rgba(7,26,47,0.06)" strokeWidth="7" />
                  <circle
                    cx="30" cy="30" r={r} fill="none" stroke="var(--color-success)" strokeWidth="7"
                    strokeDasharray={`${filled} ${circumference - filled}`}
                    strokeLinecap="round" transform="rotate(-90 30 30)"
                  />
                  <text x="30" y="35" textAnchor="middle" fill="var(--color-text)" fontSize="13" fontWeight="700" fontFamily="Manrope, system-ui, sans-serif">{score}</text>
                </svg>
                <div style={{ flex: 1, display: "grid", gap: 7 }}>
                  {segments.map(seg => (
                    <div key={seg.label}>
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 3 }}>
                        <span style={{ fontSize: 10, color: "var(--color-text-muted)" }}>{seg.label}</span>
                        <span style={{ fontSize: 10, color: seg.color, fontWeight: 700 }}>{seg.value}</span>
                      </div>
                      <div style={{ height: 4, background: "rgba(7,26,47,0.05)", borderRadius: 999, overflow: "hidden" }}>
            ...
            <div style={mockCardStyle}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 6 }}>
                {budgetItems.map(item => (
                  <div key={item.label} style={{ background: "rgba(7,26,47,0.025)", border: "1px solid rgba(7,26,47,0.05)", borderRadius: 8, padding: "7px 4px", textAlign: "center" }}>
                    <div style={{ fontSize: 12, color: item.color, fontWeight: 700, fontFamily: "var(--tekori-font-ui)", lineHeight: 1 }}>{item.value}</div>
...
function FinishSlide() {
  return (
    <div style={{ textAlign: "center", width: "100%", maxWidth: 560 }}>
      <div style={heroIconWrap}><Rocket size={34} color="var(--tekori-gold)" strokeWidth={1.6} /></div>
      <div style={eyebrowStyle("var(--tekori-gold)")}>You're Ready</div>
      <h2 style={heroTitleStyle}>Now you know where to go.</h2>
      <p style={heroBodyStyle}>Use the Hub to orient, Navi to move, and the menu tools when you need depth, structure, practice, or research.</p>
    </div>
  );
}
...
function GuidedLayout({ preview, callout, accent }: { preview: ReactNode; callout: ReactNode; accent: string }) {
  return (
    <div style={{ width: "100%", display: "flex", flexWrap: "wrap", alignItems: "center", justifyContent: "center", gap: 20 }}>
      ...
      <div style={{ width: 52, height: 52, borderRadius: "50%", border: `1px solid color-mix(in srgb, ${accent} 18%, transparent)`, background: `color-mix(in srgb, ${accent} 7%, transparent)`, display: "flex", alignItems: "center", justifyContent: "center", flex: "0 0 auto" }}>
  ...
  return (
    <div style={mockWindowStyle}>
      ...
      <div style={{ padding: 14 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 10px 14px", borderBottom: "1px solid rgba(7,26,47,0.06)", marginBottom: 10 }}>
          <Menu size={15} color="var(--color-text-muted)" />
          <div style={{ fontSize: 12, color: "var(--color-text)", fontWeight: 600 }}>Tekori Navigation</div>
        </div>
        <div style={{ display: "grid", gap: 8 }}>
          {MENU_ITEMS.map(({ label, sub, icon: Icon }) => {
            const active = label === targetLabel;
            return (
              <div key={label} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 12px", borderRadius: 12, border: `1px solid ${active ? "rgba(216,155,43,0.35)" : "rgba(7,26,47,0.06)"}`, background: active ? "rgba(216,155,43,0.12)" : "rgba(7,26,47,0.025)", boxShadow: active ? "0 0 0 1px rgba(216,155,43,0.05) inset" : "none" }}>
                <div style={{ width: 34, height: 34, borderRadius: 10, background: active ? "rgba(216,155,43,0.16)" : "rgba(7,26,47,0.05)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                  <Icon size={16} color={active ? "var(--tekori-gold)" : "var(--color-text-soft)"} />
                </div>
                <div style={{ minWidth: 0 }}>
                  <div style={{ fontSize: 12, color: active ? "var(--color-text)" : "var(--color-text-soft)", fontWeight: 600 }}>{label}</div>
                  <div style={{ fontSize: 11, color: active ? "var(--tekori-gold)" : "var(--color-text-muted)", marginTop: 2 }}>{sub}</div>
                </div>
              </div>
            );
          })}
...
function CalloutCard({ step }: { step: GuideStep }) {
  ...
  return (
    <div style={{ background: "var(--color-surface)", border: "1px solid var(--color-border)", borderRadius: 22, padding: 24, boxShadow: "0 24px 70px rgba(16,32,51,0.10)" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
        <div style={{ width: 44, height: 44, borderRadius: 14, background: `color-mix(in srgb, ${step.accent} 10%, transparent)`, border: `1px solid color-mix(in srgb, ${step.accent} 15%, transparent)`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
          <Icon size={20} color={step.accent} />
        </div>
        <div style={{ minWidth: 0 }}>
          <div style={eyebrowStyle(step.accent!)}>{step.eyebrow}</div>
          <div style={{ fontSize: 26, lineHeight: 1.2, color: "var(--color-text)", fontFamily: "var(--tekori-font-brand)", fontWeight: 700 }}>{step.title}</div>
        </div>
      </div>
      <p style={{ margin: 0, fontSize: 15, lineHeight: 1.75, color: "rgba(16,32,51,0.68)", fontFamily: "var(--tekori-font-ui)" }}>{step.body}</p>
...
function MockWindowHeader({ title }: { title: string }) {
  return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 14px", borderBottom: "1px solid rgba(7,26,47,0.06)" }}>
      ...
      <div style={{ display: "flex", gap: 6 }}>{[0, 1, 2].map((dot) => <div key={dot} style={{ width: 6, height: 6, borderRadius: 999, background: "rgba(7,26,47,0.14)" }} />)}</div>
    </div>
  );
}
...
function MockBubble({ role, text }: { role: "forge" | "user"; text: string }) {
  ...
  return (
    <div style={{ display: "flex", justifyContent: isForge ? "flex-start" : "flex-end" }}>
      <div style={{ maxWidth: "85%", padding: "12px 14px", borderRadius: isForge ? "8px 18px 18px 18px" : "18px 8px 18px 18px", background: isForge ? "rgba(7,26,47,0.045)" : "rgba(216,155,43,0.14)", border: `1px solid ${isForge ? "rgba(7,26,47,0.08)" : "rgba(216,155,43,0.28)"}`, color: isForge ? "var(--color-text-soft)" : "var(--color-text)", fontSize: 12, lineHeight: 1.65 }}>
        {text}
      </div>
    </div>
  );
